...
int FindIndex(char* str, int left, int right, char data) {
    for (int i = left; i <= right; i++) {
        if (str[i] == data) return i;
    }
    return -1;
}

btree BuildFromPreIn(char* pre, int preL, int preR, char* in, int inL,
                     int inR) {
    ...
    root->left =
        BuildFromPreIn(pre, preL + 1, preL + leftSize, in, inL, mid - 1);
    root->right =
        BuildFromPreIn(pre, preL + leftSize + 1, preR, in, mid + 1, inR);

    return root;
}
btree BuildFromPostIn(char* post, int postL, int postR, char* in, int inL,
                      int inR) {
    ...
    root->left =
        BuildFromPostIn(post, postL, postL + leftSize - 1, in, inL, mid - 1);
    root->right =
        BuildFromPostIn(post, postL + leftSize, postR - 1, in, mid + 1, inR);
...
int main() {
    ...
    if (pre[0] == 0)
        tree =
            BuildFromPostIn(post, 0, strlen(post) - 1, in, 0, strlen(in) - 1);
    if (post[0] == 0)
        tree = BuildFromPreIn(pre, 0, strlen(pre) - 1, in, 0, strlen(in) - 1);
